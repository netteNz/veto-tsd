    ...
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center">
          <Link to="/" className="mr-4 text-ink-muted transition-colors hover:text-hud">
            <ArrowLeft size={24} />
          </Link>
          <h1 className="font-display text-3xl font-bold text-ink">Random Series Generator</h1>
        </div>
      ...
      <div className="mb-6 flex items-center space-x-4 rounded-xl bg-panel p-4">
        <label className="text-sm text-ink-muted">Series Type:</label>
        <select
          value={seriesType}
          onChange={(e) => setSeriesType(e.target.value)}
          className="rounded-lg border border-transparent bg-panel-raised p-2 text-ink focus:border-hud/50"
          disabled={isLoading}
        >
      ...
      {isLoading ? (
        <div className="rounded-2xl bg-panel p-8 text-center">
          <p className="text-sm text-ink-muted">Generating random series&hellip;</p>
        </div>
      ) : randomSeries ? (
        <SeriesLayout
          series={randomSeries}
